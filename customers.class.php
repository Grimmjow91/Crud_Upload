...
// end function read_record()
    ...
    function update_record($id) { // display "update" form
        ...
        // display image upload controls: button
        echo "
...
// function select_db_record()
    ...
    function update_db_record($id) {
        ...
        if ($this->fieldsAllValid()) {
            ...
            $pdo = Database::connect();
            $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sql = "UPDATE $this->tableName  set name = ?, email = ?, mobile = ?, ";
            $sql .= "filename = ?, filesize = ?, filetype = ?, Content = ?";
            $sql .= ",description = ?, filepath = ? WHERE id = ?";
            $q = $pdo->prepare($sql);
            move_uploaded_file($this->tmpName, $fileFullPath);
            $this-> filepath = realpath('customers.class.php'); //finds the file path to the current direcotry in the server
            $this->filepath = str_replace('customers.class.php', 'uploads/', $this->filepath); //changes the directory to the uploads folder
            $this->filepath .= $this->fileName; // adds the file name to complete the path. 
            
            $q->execute(array($this->name, $this->email, $this->mobile,
                $this->fileName, $this->fileSize, $this->fileType, $this->content,
                $this->description, $this-> filepath, $this->id));           
    ...
    private function generate_html_bottom($fun) {
        switch ($fun) {
            ...
            case 3: // update
                ...
                $updateBut = '<p>Description</p><textarea rows="10" cols="35" 
...
// end function fieldsAllValid() 
    ...
    function list_records() {
        ...
        echo "
            ...
            <body>
                <a href='https://github.com/Grimmjow91/Crud_Upload' target='_blank'>Github</a><br />
                ...
                <div class='container'>
                    ...
                    <div class='row'>
                        <table class='table table-striped table-bordered' height=100px>
                            <thead>
                                <tr>
                                    <th height= 20px width=40%>Image</th>
                                    <th>Name</th>
                                    <th>Email</th>
                                    <th>Mobile</th>
                                    <th>Action</th>
